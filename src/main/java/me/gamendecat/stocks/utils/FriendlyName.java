...
public class FriendlyName {
    private static final ImmutableMap<Object, Object> map = ImmutableMap.builder()
            .put("1", "Stone")
            .put("1:1", "Granite")
    ...
    public static String lookup(ItemStack stack) {
        ...
        String key = stack.getType().createBlockData().getAsString();
        Material mat = stack.getType();
        if ((mat.toString().contains("WOOL") || mat.toString().contains("CARPET")) && stack.getDurability() == 0) {
            result = (String) map.get(key);
        } else if (mat.toString().contains("WOOL")|| mat.toString().contains("CARPET") || mat.toString().contains("TERRACOTTA") || mat.toString().contains("STAINED_GLASS") || mat.toString().contains("STAINED_GLASS_PANE")) {
            DyeColor dc = DyeColor.getByWoolData((byte)stack.getDurability());
            result = (dc == null) ? (String) map.get(key) : (WordUtils.capitalizeFully(dc.toString().replace("_", " ")) + " " + map.get(key));
            ...
            result = (dc == null) ? (String) map.get(key) : (WordUtils.capitalizeFully(dc.toString()).replace("_", " ") + " " + (String)map.get(key));
        } else if (stack.getDurability() != 0) {
            result = (String) map.get(key + ":" + stack.getDurability());
            if (result == null)
                result = (String) map.get(key);
        } else {
            result = map.containsKey(key) ? (String) map.get(key) : stack.getType().toString();
        }
        return result;
    }

    public static String mat(Material mat, short durability) {
        String result, key = mat.createBlockData().getAsString();
        if ((mat.toString().contains("WOOL") || mat.toString().contains("CARPET")) && durability == 0) {
            result = (String) map.get(key);
        } else if (mat.toString().contains("WOOL")|| mat.toString().contains("CARPET") || mat.toString().contains("TERRACOTTA") || mat.toString().contains("STAINED_GLASS") || mat.toString().contains("STAINED_GLASS_PANE")) {
            DyeColor dc = DyeColor.getByWoolData((byte)durability);
            result = (dc == null) ? (String) map.get(key) : (WordUtils.capitalizeFully(dc.toString().replace("_", " ")) + " " + (String)map.get(key));
        } else if (durability != 0) {
            result = (String) map.get(key + ":" + durability);
            if (result == null)
                result = (String) map.get(key);
        } else {
            result = map.containsKey(key) ? (String) map.get(key) : mat.toString();
        }
        return result;
    }
